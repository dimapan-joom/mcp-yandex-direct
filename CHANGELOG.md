...
### Исправлено (форк Joom)

- `audit_account_health`: проверка отклонённых объявлений не работала НИ В ОДНОМ аккаунте.
  Запрос уходил в `ads/get` с единственным критерием `Statuses`, а API требует хотя бы один
  фильтр по объектам (CampaignIds/AdGroupIds/Ids) и отвечает ошибкой 4001. Теперь проверка
  идёт пакетами по 10 кампаний — это документированный предел `CampaignIds`, подтверждённый
  на живом API (11 идентификаторов отклоняются, 10 проходят), — с пагинацией внутри пакета
  и пропуском архивных кампаний.
- Секция `rejectedAds` получила поля `campaignsChecked`, `campaignsTotal` и `complete`.
  Ноль отклонённых при неполном покрытии больше нельзя спутать с «отклонённых нет»:
  частичная проверка добавляет отдельную находку, а провалившаяся — прямо говорит, что
  вывод об отсутствии отклонений делать нельзя. Одна упавшая пачка не отменяет остальные.
